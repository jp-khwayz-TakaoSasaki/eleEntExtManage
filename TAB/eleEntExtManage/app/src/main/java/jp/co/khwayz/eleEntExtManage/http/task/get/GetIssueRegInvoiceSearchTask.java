...
public class GetIssueRegInvoiceSearchTask extends HttpTaskBase<IssueRegInvoiceSearchResponse> {

    // region [ private variable ]
    private final String mJsonParam;
    // endregion

    public GetIssueRegInvoiceSearchTask(@NonNull Callback<IssueRegInvoiceSearchResponse> callback, String paramJson) {
        super(callback);
        mJsonParam = paramJson;
    }

    @Override
    protected IssueRegInvoiceSearchResponse doInBackground() {
        IssueRegInvoiceSearchResponse result;
        try {
            // Http Post
            String url = Application.apiUrl + Constants.HTTP_SERVICE_NAME + "/syukko/registinvoice/search/";
            ...
            // JSON変換
            Gson gson = new Gson();
            result = gson.fromJson(executeResponse.getResponseJson(), IssueRegInvoiceSearchResponse.class);
            // 業務エラーの場合
            if (result.getStatus().equals(Constants.API_RESPONSE_STATUS_OPE_ERR)) {
                // -1(その他エラー)
                raiseOnError(Constants.HTTP_OTHER_ERROR, R.string.err_message_E9001);
                return null;
            }
    ...
    @Override
    protected void onPostExecute(IssueRegInvoiceSearchResponse response) {
        // エラー終了の場合
