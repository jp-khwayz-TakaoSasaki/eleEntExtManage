...
import jp.co.khwayz.eleEntExtManage.databinding.FragmentPackingOffBookBinding;
import jp.co.khwayz.eleEntExtManage.dialog_fragment.MessageDialogFragment;
import jp.co.khwayz.eleEntExtManage.util.Util;

public class PackingOffBookFragment extends BaseFragment implements RFIDDataDelegate, BarcodeDataDelegate {
    // DataBinding
    FragmentPackingOffBookBinding mBinding;
    // Adapter
    PackingScanOffBookRecyclerViewAdapter mOffBookListAdapter;
    // 画面リスト
    private RecyclerView mOffBookInfoListRecyclerView;
    private List<PackingScanInfo> mOffBookInfoList;
    // スキャナー制御
    ...
    private static final String ARGS_CARTON_COUNT = "curton_count";
    private static final String ARGS_READ_COUNT = "read_count";
    private String mInvoiceNo;
    private int mCartonCount;
    private int mReadCount;

    public static PackingOffBookFragment newInstance(String invoiceNo, int cartonCount, int readCount) {
        PackingOffBookFragment fragment = new PackingOffBookFragment();

        Bundle args = new Bundle();
        args.putString(ARGS_INVOICE_NO, invoiceNo);
        args.putInt(ARGS_CARTON_COUNT, cartonCount);
        args.putInt(ARGS_READ_COUNT, readCount);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        mBinding = FragmentPackingOffBookBinding.inflate(inflater, container, false);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        this.mCartonCount = args.getInt(ARGS_CARTON_COUNT);
        this.mReadCount = args.getInt(ARGS_READ_COUNT);
    }
    ...
    public void viewSetting() {
        ...
        // タイトル
        mListener.setSubTitleText(getString(R.string.packing) + getString(R.string.picking_invoice_scan) + getString(R.string.picking_invoice_scan_off_book));
        mListener.setScreenId(getString(R.string.screen_id_packing_offbook));

        // サブヘッダセット
        mBinding.offBookConfirmViewInvoiceNo.setText(this.mInvoiceNo);
        mBinding.offBookConfirmViewCartonCount.setText(String.valueOf(this.mCartonCount));
        mBinding.offBookConfirmViewReadCount.setText(String.valueOf(this.mReadCount));
        ...
        // リストView生成
        this.mOffBookListAdapter = new PackingScanOffBookRecyclerViewAdapter(Application.scanOffBooklList);
