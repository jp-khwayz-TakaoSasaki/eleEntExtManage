...
namespace Elematec.EntExtManage_RFIDSystem_Importer.Services
{
    ...
    /// 無償品インポート関連サービス
    /// </summary>
    public class MushouhinImporter : ImportBase
    {
        #region ■コンストラクタ■
        ...
        {
            Log.Logger.Info("無償品インポート Start");
            ...
            foreach (var file in targetFileList)
            {
                ...
                if (executeArgument.RtnCode == 9)
                {
                    ...
                    {
                        TorikomiKubn = TorikomiKubun.Mushouhin,
                        TargetFileFile = file,
                    ...
                    this.MoveFaildFile(importFilePath);
                    break;
                }
                else
                {
        ...
        private ExecuteArgument ExecuteImport(string importFilePath, string errorFilePath)
        {
            ...
            using (var db = new Database(dbConnectionString))
            {
                ...
                executeArgument = nyukoYotei.ImportByMushouhin(db, importFilePath, errorFilePath);
            }
        ...
        private ExecuteArgument RegistNyukoYotei()
        {
            ...
            try
            {
                using (var db = new Database(dbConnectionString))
                {
                    W_MUSHOUHIN_TEMP w_Mushouhin_Temo = new W_MUSHOUHIN_TEMP();

                    w_Mushouhin_Temo.GetRecordCount(db);
            ...
            try
            {
                ...
                {
                    T_NYUKOYOTEI nyukoYotei = new T_NYUKOYOTEI();
                    executeArgument = nyukoYotei.UpdateByMushouhin(db);
                }
